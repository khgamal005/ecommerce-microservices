...
const ForgetPasswordForm = () => {
  ...
  const resetPasswordMutation = useMutation({
    ...
    onError: (err: any) => setServerError(err.message || 'Server error. Please try again.'),
  });
  ...
  {errors.email?.message ? String(errors.email.message) : null}
            ...
            >
              {forgetPasswordMutation.isPending && <Loader2 className="animate-spin mr-2" />}
      ...
      {step === 2 && (
        <>
          <h2 className="text-2xl font-semibold mb-5 text-center">Verify OTP</h2>
          ...
          <div className="flex justify-center gap-3 mb-4">
            {otp.map((digit, index) => (
              <input
                ...
                ref={(el: HTMLInputElement | null) => (inputsRefs.current[index] = el)}
          ...
          >
            {verifyOtpMutation.isPending && <Loader2 className="animate-spin mr-2" />}
            {verifyOtpMutation.isPending ? 'Verifying...' : 'Verify OTP'}
          </button>
          <div className="text-center">
            {canResend ? (
              <button onClick={() => resendOtpMutation.mutate()} className="text-blue-500 hover:underline text-sm">
                Resend OTP
              </button>
            ) : (
              <p className="text-sm text-gray-600">
                Resend OTP in <span className="text-blue-500 font-semibold">{timer}</span>s
              </p>
            )}
          </div>
        </>
      ...
      {step === 3 && (
        <>
          <h2 className="text-2xl font-semibold mb-5 text-center">Reset Password</h2>
          <form onSubmit={handleSubmit(onResetSubmit)}>
            ...
            <button
              ...
              className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white py-2 rounded-lg flex items-center justify-center"
            >
              {resetPasswordMutation.isPending && <Loader2 className="animate-spin mr-2" />}
