...
const RegisterForm = () => {
  ...
  const [registerLoading, setRegisterLoading] = useState(false); // Loading state for registration
  ...
  const onSubmitOtp = async () => {
    ...
    try {
      ...
      setLoading(false); // Set loading to false when OTP verification ends
    }
  };
      ...
      {!showOtp ? (
        ...
        >
          ...
          <button
            ...
            className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white py-2 rounded-lg font-medium transition-colors duration-200 mb-3 flex items-center justify-center"
          >
            {registerLoading ? <Loader2 className="animate-spin mr-2" size={20} /> : null}
        ...
        <div className="w-full max-w-md mx-auto p-6 shadow-lg border rounded-xl bg-white">
          ...
          <div className="space-y-4">
            <button
              onClick={onSubmitOtp}
              disabled={loading}
              className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white py-3 rounded-lg font-medium transition-colors duration-200 cursor-pointer flex items-center justify-center"
            >
            ...
            <div className="text-center">
              {canResend ? (
                <button
                  onClick={ResendOtp}
                  className="text-blue-500 hover:text-blue-600 cursor-pointer font-medium text-sm hover:underline transition-colors"
                >
                  Resend OTP
                </button>
              ) : (
                <p className="text-gray-600 text-sm">
                  Resend OTP in <span className="font-medium text-blue-500">{timer}</span> seconds
                </p>
              )}
            </div>
          </div>
