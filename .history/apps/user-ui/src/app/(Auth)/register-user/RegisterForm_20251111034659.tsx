...
const RegisterForm = () => {
  ...
  const registerMutation = useMutation({
    mutationFn: async (data: ) => {
      const res = await fetch('http://RegisterFormInputslocalhost:6001/api/register-user', {
        method: 'POST',
  ...
  const resendOtpMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch('http://localhost:6001/api/resend-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="w-full min-h-screen flex items-center justify-center bg-gray-50 py-8">
      {!showOtp ? (
        ...
        <div className="w-full max-w-md mx-auto p-6 shadow-lg border rounded-xl bg-white">
          <h3 className="text-xl font-semibold text-center mb-6">Verify Your Email</h3>
