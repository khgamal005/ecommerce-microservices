...
export const loginUser = async (
  ...
  try {
    ...
    const accessToken = jwt.sign({ id: user.id, role: user }, process.env.JWT_SECRET!, { expiresIn: '1h' });
    const refre = jwt.sign({ id: user.id, role: user }, process.env.JWT_SECRET!, { expiresIn: '1h' });
    // ✅ Respond
