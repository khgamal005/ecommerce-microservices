...
export const refreshToken = async (
  ...
  try {
   ...
   const   decoded = jwt.verify(token, process.env.REFRESH_TOKEN_SECRET!) as{id :string,role:string };
   if(!decoded || !decoded.id || !decoded.role){
    return new JsonWebTokenError(" forbiden invalid token")
    ...
    if (!user) {
      return next(new AuthError('User/seller not found'));
    }
    ...
    // Set cookies
    setAuthCookies(res, {     const newAccessToken = jwt.sign({ id: decoded.id ,role:decoded.role }, process.env.JWT_SECRET!, {
, refreshToken });
