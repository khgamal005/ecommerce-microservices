...
export const verifyOtp = async (
  ...
  const faildAtempts = `otp_failed_attempts:${email}`;

  let otpFailedAttempts = parseInt((await redis.get(faildAtempts)) || '0');

  if (savedOtp !== otp) {
    if (otpFailedAttempts >= 3) {
      await redis.set(`otp_lock:${email}`, 'locked', 'EX', 1800);
      await redis.set(faildAtempts, otpFailedAttempts.toString(), 'EX', 1800);
    ...
    await redis.set(faildAtempts, otpFailedAttempts + 1, 'EX', 300);
    return next(
      new ValidationError(
