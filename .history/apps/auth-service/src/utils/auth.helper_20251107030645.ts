...
export const verifyOtp = async (
  ...
  const otpFailedAttempts = `otp_failed_attempts:${email}`;

  let otpFailedAttempts = parseInt((await redis.get(otpFailedAttempts)) || '0');

  if (savedOtp !== otp) {
    if (otpFailedAttempts >= 3) {
      await redis.set(`otp_lock:${email}`, 'locked', 'EX', 1800);
      await redis.set(otpFailedAttempts, otpFailedAttempts.toString(), 'EX', 1800);
    ...
    await redis.set(otpFailedAttempts, otpFailedAttempts + 1, 'EX', 300);
    return next(
      new ValidationError(
  ...
  await redis.del(otpFailedAttempts);
};
