...
export const verifyOtp = async (email: string, otp: string, next: NextFunction) => {
  ...
  if (!savedOtp) {
    return next(new ValidationError("OTP expired or not found"));
  }
  if (savedOtp !== otp) {
    return next(new ValidationError("Invalid OTP"));
  }
  const otpRequestKey = `otp_failed_attempts:${email}`;

  let otpFailedAttempts = parseInt((await redis.get(otpRequestKey)) || '0');

  if (savedOtp !== otp) {
    if (otpFailedAttempts >= 3) {
      await redis.set(`otp_lock:${email}`, 'locked', 'EX', 1800);
      await redis.set(otpRequestKey, otpFailedAttempts.toString(), 'EX', 1800);
      await redis.del(`otp:${email}`);
      return next(new ValidationError("Account locked due to too many failed OTP attempts. Please try again after 30 minutes."));
    }

      await redis.set(otpRequestKey, otpFailedAttempts + 1, 'EX', 300);
    return next(new ValidationError(`Invalid OTP. You have ${3 - otpFailedAttempts} attempts left.`));
  }
        await redis.del(`otp:${email}`,otpRequestKey);
