...
export const verifyOtp = async (
  ...
  const failedAttemptsOtp = `otp_failed_attempts:${email}`;

  let otpFailedAttempts = parseInt((await redis.get(failedAttemptsOtp)) || '0');

  if (savedOtp !== otp) {
    if (otpFailedAttempts >= 3) {
      await redis.set(`otp_lock:${email}`, 'locked', 'EX', 1800);
      await redis.set(failedAttemptsOtp, otpFailedAttempts.toString(), 'EX', 1800);
    ...
    await redis.set(failedAttemptsOtp, otpFailedAttempts + 1, 'EX', 300);
    return next(
      new ValidationError(
  ...
  await redis.del(failedAttemptsOtp);
};
