...
export const validationRegistrationUser = (
  ...
  console.log("🔍 validationRegistrationUser received data:", data);
  
  // Use 'name' instead of 'username' to match your request body
  const { email, password, name, phone_number, country } = data;

  console.log("🔍 Extracted fields:", { email, password, name, phone_number, country });
...
export const sendotp = async (
  ...
  await redis.set(`otp_cooldown:${email}`, 'true', 'EX', 60); // Attempts counter valid for  minutes
};
