...
export const handelForgetPassword = async (
  req : Request,
  res :Response ,
  next :NextFunction,
  userType : 'user' | 'seller') => {
    ...
    try{
      ...
      await sendotp(email,user.name,userType==='user' &&'forget-password-user-mail')
      res.status(200).json("otp sent to your email verify you")

      // const seller = userType === 'seller' && await prisma.seller.findUnique({ where: { email } });
      // if (!seller) {
