...
export const verifyOtp = async (
  ...
  const faildA = `otp_failed_attempts:${email}`;

  let otpFailedAttempts = parseInt((await redis.get(faildA)) || '0');

  if (savedOtp !== otp) {
    if (otpFailedAttempts >= 3) {
      await redis.set(`otp_lock:${email}`, 'locked', 'EX', 1800);
      await redis.set(faildA, otpFailedAttempts.toString(), 'EX', 1800);
    ...
    await redis.set(faildA, otpFailedAttempts + 1, 'EX', 300);
    return next(
      new ValidationError(
